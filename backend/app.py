...
from strategy_manager import STRATEGY_METADATA, StrategyManager  # noqa: E402
...
class StrategyRuntime:
    ...
    def _build_command(self, config: Dict) -> List[str]:
        cmd = ['python3', str(SCRIPTS_DIR / 'strategy_manager.py'), '--strategy', config['strategy']]
        for code in config.get('codes') or []:
            if '--codes' not in cmd:
                cmd.extend(['--codes', code])
            else:
                cmd.append(code)
